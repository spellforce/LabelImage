...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        autolabel = action(get_str('autoLabel'), self.auto_label,
                        'Ctrl+P', 'autolabel', get_str('autoLabelDetail'))
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, verify, save, save_format, autolabel, create, copy, delete, None,
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if self.default_save_dir is not None:
            ...
            if os.path.isfile(xml_path):
                ...
                # autolabel
                self.load_yolo_txt_by_filename_auto()
    ...
    def import_dir_images(self, dir_path):
        ...
        if self.cur_img_idx != 0: self.file_path = self.m_img_list[self.cur_img_idx]
        else: self.file_path = None
        self.file_list_widget.clear()
    ...
    def auto_label(self):
        ...
        # if attention_get == QMessageBox.Yes:
            path = os.path.join(os.path.dirname(ustr(self.file_path)), os.pardir) if self.file_path else '.'
            if not self.last_open_dir:
                self.last_open_dir='./images'
            weight_path = QFileDialog.getOpenFileName(self, "getOpenFileName", path,  "Weight Files (*.pt)")
            ...
            weightFile = weight_path[0]
            source = self.last_open_dir
            print(source)
            self.model =  torch.hub.load('ultralytics/yolov5', 'custom', path=weightFile)
    ...
    def load_yolo_txt_by_filename_auto(self):
        ...
        for *box, conf, cls in results.xyxy[0]: #iterate over each player detected
            ...
            x1, y1, conf = *x1y1, conf.item()
            x2, y2 = x2y2[0], x2y2[1]
            cls = classNames[int(cls)]
            shapes.append((cls, [(x1, y1), (x2, y1), (x2, y2), (x1, y2)], None, None, False))
        # shapes = [('head', [(184, 152), (202, 152), (202, 169), (184, 169)], None, None, False)]
        print(shapes, "by spellforce auto label")
        self.load_labels(shapes)
        # self.canvas.set_editing(True)
        if len(shapes) > 0: self.dirty = True
        # self.canvas.verified = t_yolo_parse_reader.verified
